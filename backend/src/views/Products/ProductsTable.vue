<template>
  <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
    ...
    <table class="table-auto w-full">
      ...
      <tbody v-else>
<!--            <tr v-for="(product, index) of products.data" class="shadow animate-fade-in-down" :style="{'animation-delay': `${index * 0.1}s`}" >-->
      <tr v-for="(product, index) of products.data">
        <td class="border-b p-2"> {{ product.id }}</td>
        <td class="border-b p-2">
          <img v-if="product.image_url" class="w-16" :src="product.image_url" :alt="product.title">
          <img v-else class="w-16 h-16 object-cover" src="../../assets/no-image.jpeg" :alt="product.title">
        </td>
